...
export default function AIChatOverlay() {
  ...
  // Reset messages when role changes (only relevant for merchants – consumers use ChatContext on /chat)
  useEffect(() => {
  ...
  const sendMessage = () => {
    ...
    // For consumer users, always use the global /chat page (ChatContext) so conversation is persisted
    if (!isMerchant) {
      // Ensure we start from a clean chat session for consumers; Chat.tsx will handle sending the query
      resetChat();
      navigate(`/chat?q=${encodeURIComponent(query)}`);
      return;
    }

    // For merchant users, respond inside this overlay
