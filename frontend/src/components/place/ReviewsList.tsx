...
interface Props {
  placeId: string;
  /** When provided (e.g. from crawled Zurich data), use these instead of fetching. */
  initialReviews?: PlaceReview[] | null;
}

export default function ReviewsList({ placeId, initialReviews }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    if (initialReviews != null && initialReviews.length > 0) {
