...
- `rating_mode`（可选）：控制前端详情页评分分布所使用的数据源。
  - `"apify_raw"`（默认）：使用 Apify 爬虫返回的原始评分直方图（全量或平台曝光的总评分分布）。
  - `"review_pipeline"`：使用 review_analysis 管道基于最近一批被分析的文本评论计算出来的评分分布（通常为一个较小的采样窗口，如最近 1 年内最多 N 条评论），用于更“近期偏置”的分析视图。
